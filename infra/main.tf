...
module "network" {
  ...
  count                        = (var.is_secure_mode && !var.bring_your_own_vnet) ? 1 : 0
...
module "existing_network" {
  source                      = "./core/network/existingNetwork"
  count                       = (var.is_secure_mode && var.bring_your_own_vnet) ? 1 : 0
  vnet_name                   = var.vnet_name
  resourceGroupName           = var.resourceGroupName
  web_subnet_name             = var.web_subnet_name
  web_subnet_integration_name = var.web_subnet_integration_name
  subnet_service_name         = var.subnet_service_name
  subnet_data                 = var.subnet_data
  subnet_openai               = var.subnet_openai
}

// Create the Private DNS Zones for all the services
module "privateDnsZoneAzureOpenAi" {
  source             = "./core/network/privateDNS"
  count              = var.is_secure_mode ? 1 : 0 && var.create_private_dns_zone ? 1 : 0
